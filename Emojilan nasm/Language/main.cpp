...
const char* CODE_FILES1 = "../../resources/savefiletocodeasm.s";
        ...
        if ((cell->nextr == NULL) && (cell->nextl == NULL)) {
            if (('a' <= cell->data [0]) && (cell->data [0] <= 'z')) {
                ...
                for (int i = 5; i < len + 5; ++i) {
                    str [i] = '0' + num / (Ten (len - i + 5));
                    num = num - (num / (Ten (len - i + 5)) * (Ten (len - i + 5)));
                }
                
                str [len + 5] = ']';
                str [len + 6] = '\0';
                memcpy(cell->data, str, len + 7);
